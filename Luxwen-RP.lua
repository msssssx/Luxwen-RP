...
local filter = {
	...
	"^{00a1fa}%[Информация%] {ffffff}• Ознакомьтесь с ними! %- gta%-lux%.ru",
	"^Сейчас времени",
	...
	"^{00a1fa}%[Информация%] {ffffff}GTA%-LUX%.RU Форум: {8BD032}gta%-lux%.ru",
	...
	"^{FFAF00}SMS: {FFF700}Привет, друг, желаешь получить донат рубли или билеты на контейнер%? {FFAF00}| {FFF700}Отправитель: .+$",
	"^{FFAF00}SMS: {FFF700}Мы ищем опытных дальнобойщиков на работу! {FFAF00}| {FFF700}Отправитель: .+$",
	"^{FFAF00}SMS: {FFF700}Мы в навигаторе /gps %- 5 %- \"Доставщик Материалов\"%. {FFAF00}| {FFF700}Отправитель: .+$",
	"^{FFAF00}SMS: {FFF700}Платим коинами%. Их ты сможешь обменять в будущем с помощью /coins%. {FFAF00}| {FFF700}Отправитель: .+$",
	...
	"^{FFFF00}%[Подсказка%] {ffffff}Вы отказались от ответа на репорт$",
	"^Не флуди!$"
}
...
function sampev.onSendClickPlayer(playerId, source)
	lua_thread.create(function()
		...
		if flags.is_player_bot then
			...
			sampSendChat("/re " .. playerId .. "      ") -- Пробелы это фикс рекурсии, не убирать
		end
	end)
	return false
end
	...
	elseif flags.check_report and dialogId == 5753 then
		...
		for line in text:gmatch("[^\n\r]+") do
			if number_line == 3 then
				...
				sampAddChatMessage("[A] Жалоба от " .. report.nick .. "[" .. report.id .. "]: " .. report.text, 0xff9945)
				break
			end
			number_line = number_line + 1
		end
...
function sampev.onServerMessage(color, text)
	...
	if text:find("^{ffa500}%[A%] : .*{ffa500}Поступила жалоба") then
		report.nick, report.id = text:match("{ffa500}%[A%] : .*{ffa500}Поступила жалоба от (.+)%[(%d+)%]")
		flags.check_report = true
		sampSendChat("/arep")

	elseif text:find("^{ffa500}%[A%] : {ffa500}Поступил новый репорт от") then
		report.nick, report.id = text:match("{ffa500}%[A%] : {ffa500}Поступил новый репорт от (.+)%[(%d+)%], ")
		flags.check_report = true
		sampSendChat("/arep")

	elseif text:find("^Данный репорт уже взяли$") then
		lua_thread.create(function()
			wait(1000)
			flags.check_report = true
			sampSendChat("/arep")
	...
	elseif flags.check_report and text:find("^Репорты не найдены%.$") then
		return false
	...
	elseif text:find("^%[A%]") or text:find("^{AFAFAF}%[A%]") and not text:find("%[A%] Игрок: .+ и получил мут!$") then
		...
		if color == -1347440726 or color == -1347440641 or color == -1007741441 or color == -1263159297 or color == -6750038 or text:find("%[%d+ HP%]$") or text:find("освободил игрока .+ из тюрьмы$") then
			color = 0xFFFFFFFF
