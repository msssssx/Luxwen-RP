...
local filter = {
	...
	"^{FFAF00}SMS: {FFF700}Привет, друг, желаешь получить донат рубли или билеты на контейнер%? {FFAF00}| {FFF700}Отправитель: Cavallaro$",
	"^{FFAF00}SMS: {FFF700}Мы ищем опытных дальнобойщиков на работу! {FFAF00}| {FFF700}Отправитель: Cavallaro$",
	"^{FFAF00}SMS: {FFF700}Мы в навигаторе /gps %- 5 %- \"Доставщик Материалов\"%. {FFAF00}| {FFF700}Отправитель: Cavallaro$",
	"^{FFAF00}SMS: {FFF700}Платим коинами%. Их ты сможешь обменять в будущем с помощью /coins%. {FFAF00}| {FFF700}Отправитель: Cavallaro$",
	"^Администратор .+: Самые дешевые админки только у нас! Посмотреть цены можно в /buyadm$",
	"^Администратор .+: Для пополнения счета перейдите на наш сайт %- gta%-lux%.ru, раздел %- Донат$",
	"^Администратор .+: Действует Х2 донат закинул 500 пришло 1150 :%)$",
	"^Администратор .+: Всем приятной игры и великолепного времяпровождения на нашем сервере :%)$"
}
...
function main()
	...
	while true do
		if flags.is_spectator and not sampIsChatInputActive() and wasKeyPressed(keys.VK_SPACE) then -- Выйти из наблюдения нажатием пробела
			sampSendChat("/re off")
		end
		wait(0)
...
function sampev.onSendClickPlayer(playerId, source)
	lua_thread.create(function()
		...
		if flags.is_player_bot then
			sampAddChatMessage("[A] Следить за ботом {FFFFFF}" .. sampGetPlayerNickname(playerId) .. "[" .. playerId .. "]  {0373fc}невозможно.", 0x0373FC)
		else
...
function sampev.onRemovePlayerFromVehicle()
	return false	-- Ездим в любой машине
end

-- =================================================================================

-- function sampev.onCreatePickup(id, model, pickupType, position)
	-- sampAddChatMessage("Создан пикап {FFFFFF}[id: " .. tostring(id) .. ", model: " .. tostring(model) .. ", pickupType: " .. tostring(pickupType) .. ", position: " .. tostring(position) .. "]", 0x00FF00)
...
-- function sampev.onSetRaceCheckpoint(type, position, nextPosition, size)
	-- sampAddChatMessage("Создан чекпоинт {FFFFFF}[type: " .. tostring(type) .. ", position: " .. position.x .. "|" .. position.y .. "|" .. position.z .. ", nextPosition: " .. tostring(nextPosition) .. ", size: " .. tostring(size) .. "]", 0x00FF00)
...
function sampev.onShowDialog(dialogId, style, title, button1, button2, text)  -- (5752 - /arep) (5753 - текст репорта)
	...
	if flags.check_report and dialogId == 5752 then
		...
		for line in text:gmatch("[^\n\r]+") do
			if line:find(report.nick) then
				sampSendDialogResponse(5752, 1, number_line, "")
				break
			end
	...
	elseif flags.check_report and dialogId == 5753 then
		local number_line = 0
		for line in text:gmatch("[^\n\r]+") do
			if number_line == 4 then
				report.text = line
				sampSendDialogResponse(5753, 0, 1, "")
				sampAddChatMessage("[A] Жалоба от " .. report.nick .. "[" .. report.id .. "]: " .. report.text, 0xFF6600)
				break
			end
			number_line = number_line + 1
		end
	...
	-- if dialogId == 9623 then
		-- local number_line = 1
		-- for line in text:gmatch("[^\n\r]+") do
			-- if line:find("Статистика") then
				-- -- sampAddChatMessage(number_line, -1)
				-- sampSendDialogResponse(9623, 1, number_line - 1, "")
				-- break
			-- end
			-- number_line = number_line + 1
		-- end
	...
	-- elseif dialogId == 1932 then
		-- local number_line = 1
		-- for line in text:gmatch("[^\n\r]+") do
			-- if number_line == 27 then
				-- stats.rub = line:match("(%d+)")
				-- -- sampSendDialogResponse(5753, 1, 1, "")
				-- -- sampSendDialogResponse(5752, 1, 1, "")
				-- sampAddChatMessage("Рублей: {FFFFFF}" .. stats.rub, 0xFF6600)
				-- break
			-- end
			-- number_line = number_line + 1
		-- end
...
function sampev.onSendCommand(cmd)
	...
	elseif not flags.show_news and cmd:find(string.lower("^/aedit")) then
		sampAddChatMessage("[A] Сначала включите отображение новостей (/show_news).", 0xfc8403)
		return false
	
	elseif cmd:find(string.lower("^/re %d+$")) then -- BUG?
		...
		lua_thread.create(function()
			...
			if flags.is_player_bot then
				sampAddChatMessage("[A-RECON] Следить за ботом {FFFFFF}" .. sampGetPlayerNickname(id) .. "[" .. id .. "]  {fc8403}невозможно.", 0xfc8403)
			else
			-- BUG:
				-- if flags.is_player_afk then
					-- sampAddChatMessage("[A-RECON] Игрок {FFFFFF}" .. sampGetPlayerNickname(id) .. "[" .. id .. "]  {fc8403}AFK, попробуйте позже.", 0xfc8403)
				-- else
					sampSendChat("/re " .. id .. "      ") -- Пробелы это фикс рекурсии, не убирать
				-- end
			end
		end)
		return false
...
function sampev.onServerMessage(color, text)
	...
	if text:find("^{ffa500}%[A%] : .*{ffa500}Поступила жалоба") then
		report.nick = text:match("{ffa500}%[A%] : .*{ffa500}Поступила жалоба от (.+)%[%d+%],")
	...
	elseif flags.active_check_bot then
		...
		elseif text:find("^.+ %[%d+%] .*{DC143C}%[SA%-MP Mobile%]$") or text:find(".+ %[%d+%] .*{318CE7}%[PC%]") then
			flags.is_player_afk = text:find("{FF0000}%(AFK:")
			local nick, id, color, client = text:match("(.+) %[(%d+)%] .*{(.+)}%[(.+)%]")
			sampAddChatMessage("[A] Вы начали наблюдение за {FFFFFF}" .. nick .. "[" .. id .. "] {fc8403}(Клиент: {" .. color .. "}" .. client .. "{fc8403})", 0xfc8403)
			flags.is_player_bot = false
			return false
		elseif text:find("^Игроки онлайн:$") then
			return false
		end

	elseif text:find("^{FFAF00}SMS:") then
		if text:find("Отправитель: .+%[%d+%]$") then
			local message, from = text:match("{FFAF00}SMS: {FFF700}(.+) {FFAF00}| {FFF700}Отправитель: (.+)")
			sampAddChatMessage("SMS от {FF7700}" .. from .. "{FFFF00}: " .. message, 0xFFFF00)
		else
			local message, to = text:match("{FFAF00}SMS: {FFF700}(.+) {FFAF00}| {FFF700}Получатель: (.+)")
			sampAddChatMessage("SMS к {FF7700}" .. to .. "{FFFF00}: " .. message, 0xFFFF00)
		end
		return false
	...
	elseif not flags.show_news and (text:find("^{FA8D20}.+ | Контакт: ") or text:find("^  {D16E0B}Объявление отредактировано") or text:find("^СМИ | ") or text:find("^  Объявление проверил") or text:find("^%[СМИ%] Пришло новое объявление! | Смотрите: /aedit$")) then
		return false
	...
	elseif text:find("тихо кикнул") or text:find("выпустил из ада") then
		sampAddChatMessage(color, -1)

	elseif text:find("^%[A%] Текст обращения .+:.+") then
		text = text:gsub("обращения (.+): ", "обращения %1: {FFFFFF}")

	elseif text:find("^%[A%]") or text:find("^{AFAFAF}%[A%]") and not text:find("%[A%] Игрок: .+ и получил мут!$") then
		...
		if color == -1347440726 or color == -1347440641 or color == -1007741441 or text:find("%[%d+ HP%]$") or text:find("освободил игрока .+ из тюрьмы$") then
			color = 0xFFFFFFFF
...
function calculate(text) -- Калькулятор
