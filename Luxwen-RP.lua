...
local font = renderCreateFont('Arial', 12, font_flag.BOLD + font_flag.SHADOW)
...
function sampev.onSendClickPlayer(playerId, source)
	lua_thread.create(function()
		...
		if is_bot then
			sampAddChatMessage("[A] Следить за ботом " .. sampGetPlayerNickname(playerId) .. "[" .. playerId .. "]  невозможно.", 0xFF0373fc)
		else
...
function sampev.onServerMessage(color, text)
	...
	if flags.active_check_bot then
	...
	elseif text:find("^{FFCC00}Авторизация прошла успешно%.$") then
		sampSendChat("/rub")
